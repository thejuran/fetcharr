...
async def insert_search_entry(
    db_path: Path, app: str, queue_type: str, item_name: str
) -> None:
    ...
    async with aiosqlite.connect(db_path) as db:
        await db.execute(
            ...
            (timestamp, app, queue_type, item_name),
        )
        await db.execute(
            """
...
async def get_recent_searches(db_path: Path, limit: int = 50) -> list[dict]:
    ...
    Returns:
        List of dicts with keys: name, timestamp, app, queue_type.
        Ordered newest-first (by id DESC).
    """
    async with aiosqlite.connect(db_path) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
            "SELECT timestamp, app, queue_type, item_name FROM search_history ORDER BY id DESC LIMIT ?",
            (limit,),
        )
